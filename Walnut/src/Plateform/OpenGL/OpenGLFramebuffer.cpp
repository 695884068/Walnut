...
namespace Walnut
{
	...
	namespace Utils
	{
		...
		static bool IsDepthFormat(FramebufferTextureFormat format)
		{
			switch (format)
			{
			case Walnut::FramebufferTextureFormat::DEPTH32F_TEX3D:
			case Walnut::FramebufferTextureFormat::DEPTH24STENCIL8:
				return true;
				break;
			}
    ...
    void OpenGLFramebuffer::Invalidate()
    {
		...
		{
			WN_CORE_ASSERT(mColorAttachments.size() <= 4);
		...
		WN_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
    {
        ...
        {
            WN_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
            return;
        }
	...
	{
		WN_CORE_ASSERT(attachmentIndex < mColorAttachments.size());
