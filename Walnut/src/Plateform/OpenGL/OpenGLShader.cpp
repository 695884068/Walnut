...
namespace Walnut {
	...
	namespace Utils
	{
		static GLenum ShaderTypeFromString(const std::string& type)
		{
			...
			WN_CORE_ASSERT(false, "Unknown shader type!");
			return 0;
		}
		...
		static ShaderUniformType SPIRTypeToShaderUniformType(spirv_cross::SPIRType type)
		{
			...
			WN_CORE_ASSERT(false, "Unknown type!");
		...
		: mFilePath(filepath)
	{
...

		{
			...
			CreateProgram();
			WN_CORE_WARN("Shader creation took {0} ms", timer.ElapsedMillis());
		}
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			WN_CORE_ERROR("Could not open file '{0}'", filepath);
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			WN_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1; //Start of shader type name (after "#type " keyword)
			std::string type = source.substr(begin, eol - begin);
			WN_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol); //Start of shader code after shader type declaration line
			WN_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					WN_CORE_ERROR(module.GetErrorMessage());
					WN_CORE_ASSERT(false);
				}
	...
	void OpenGLShader::CompileOrGetOpenGLBinaries()
	{
		...
		for (auto& [bufferName, buffer] : mBuffers)
		{
			for (auto& [name, uniform] : buffer.Uniforms)
			{
				...
				if (location == -1)
					WN_CORE_WARN("{0}: could not find uniform location {0}", name);
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			WN_CORE_ERROR("Shader linking failed ({0}):\n{1}", mFilePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		WN_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), mFilePath);
		WN_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		WN_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		WN_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			WN_CORE_TRACE("    Name = {0}", resource.name.c_str());
			WN_CORE_TRACE("    Size = {0}", bufferSize);
			WN_CORE_TRACE("    Binding = {0}", binding);
			WN_CORE_TRACE("    Members = {0}", memberCount);
		}

		WN_CORE_TRACE("Uniform variables:");
		for (const auto& resource : resources.push_constant_buffers)
		{
			...
			WN_CORE_TRACE("    Name = {0}", resource.name.c_str());
			WN_CORE_TRACE("    Name = {0}", compiler.get_name(resource.base_type_id));
			WN_CORE_TRACE("    Name = {0}", compiler.get_fallback_name(resource.base_type_id));
			//WN_CORE_TRACE("    Name = {0}", compiler.get_name(resource.base_type_id));
			WN_CORE_TRACE("    Size = {0}", bufferSize);
			WN_CORE_TRACE("    Binding = {0}", binding);
			WN_CORE_TRACE("    Members = {0}", memberCount);
	...
	{
		WN_CORE_ASSERT(mUniformLocations.find(fullname) != mUniformLocations.end());
	...
	std::unordered_map<GLenum, std::string> NativeOpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			std::string type = source.substr(begin, eol - begin);
			WN_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void NativeOpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				WN_CORE_ERROR("{0}", infoLog.data());
				WN_CORE_ASSERT(false, "Shader compilation failure!")
					break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			WN_CORE_ERROR("{0}", infoLog.data());
			WN_CORE_ASSERT(false, "NativeOpenGLShader link failure!")
				return;
		}
